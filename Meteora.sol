...
contract Meteora {
    int constant ringMember = 3; //v from wallet + 1
    ...
    function checkKeyImage(
        ...
        for (uint256 index = 0; index < usedKeyImages.length; index++) {
            ...
            if (
                keyImage[0] == usedKeyImage[0] && keyImage[1] == usedKeyImage[1]
            ) {
                return false;
            }
        }
    ...
    function point2String(
        uint256[2] memory _point
    ) internal pure returns (string memory) {
        //has to be exactly like in wallet
        return
            string.concat(
                Strings.toString(_point[0]),
                Strings.toString(_point[1])
            );
    }
